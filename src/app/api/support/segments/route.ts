...
/** Segment master (see BRS §4.3). Owned by the support lead, so gated on the menu grant
...
export async function POST(request: NextRequest) {
  const { session, error } = await guardMenu(SUPPORT_MENU.SEGMENTS, DENIED);
  if (error) return error;
  try {
    const body = masterListSchema.parse(await request.json());
    const created = await prisma.segmentMaster.create({ data: { ...body, createdBy: session.username } });
